...
//InstallAndInstantiateCC 用于安装初始化链码
func (setup *FabricSetup) InstallAndInstantiateCC() error {
	...
	resp, err := setup.admin.InstantiateCC(setup.ChannelID, resmgmt.InstantiateCCRequest{Name: setup.ChainCodeID, Path: setup.ChaincodeGoPath, Version: "0", Args: [][]byte{}, Policy: ccPolicy})
...
func (setup *FabricSetup) UpdateCC(version string) error {
	...
	_, err = setup.admin.InstallCC(installCCReq, resmgmt.WithRetry(retry.DefaultResMgmtOpts))
	ccPolicy := cauthdsl.SignedByAnyMember([]string{"Org1MSP"})
	req := resmgmt.UpgradeCCRequest{Name: setup.ChainCodeID, Version: version, Path: setup.ChaincodePath, Policy: ccPolicy, Args: [][]byte{}}
	_, err = setup.admin.UpgradeCC(setup.ChannelID, req)
	if err != nil {
		return errors.WithMessage(err, "failed to update chaincode")
	}
	return nil
}
...
func (setup *FabricSetup) RegisterUser(userName string) error {
	mspClient, err := mspclient.New(setup.sdk.Context())
	if err != nil {
		return errors.WithMessage(err, "failed to create MSP client")
	}
	_, err = mspClient.GetSigningIdentity(setup.OrgAdmin)
	if err != nil {
		return errors.WithMessage(err, "failed to get admin signing identity")
	}
	enrollmentSecret, err := mspClient.Register(&mspclient.RegistrationRequest{Name: userName, Type: "user"})
	if err != nil {
		return errors.WithMessage(err, "failed to register user")
	}
	err = mspClient.Enroll(userName, mspclient.WithSecret(enrollmentSecret))
	if err != nil {
		return errors.WithMessage(err, "failed to enroll user")
	}
	fmt.Println("enroll user is completed,username:%s,password:%s", userName, enrollmentSecret)
	return nil
}
